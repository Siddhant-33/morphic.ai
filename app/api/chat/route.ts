...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (isGuest) {
      ...
      if (!guestChatEnabled) {
        return new Response(JSON.stringify({ error: "Please sign in to continue", requiresAuth: true }), { 
          status: 401,
    ...
    const searchMode: SearchMode = searchModeCookie && ['quick', 'adaptive', 'analyzing', 'image'].includes(searchModeCookie) 
      ? searchModeCookie as SearchMode 
      : 'quick'

    const selectedModel: any = await selectModel({ searchMode, cookieStore, taskType: 'chat' })
    ...
    const response = isGuest
      ? await createEphemeralChatStreamResponse({ messages: Array.isArray(messages) ? messages : [], model: modelForAPI, abortSignal, searchMode, chatId })
      : await createChatStreamResponse({ message, model: modelForAPI, chatId, userId: userId!, trigger, messageId, abortSignal, isNewChat, searchMode })
